...
# create logger
logger = logging.getLogger('extremecooling4linux')
logging.basicConfig(level=logging.INFO)
APP ='extremecooling4linux'
    ...
    def load_window(self):
        ...
        if os.getuid() == 0:
            ...
            if getattr(sys, '_MEIPASS', ''):
                self.enable_ec_switch_button.set_sensitive(False)
                dialog = DialogUI()
                dialog.create_dialog(_("User does not have enough privileges."), _("ExtremeCooling4Linux needs root privileges to enable/disable extreme cooling. Please, execute ExtremeCooling4Linux with sudo."),
...
class App:
    ...
    def pre(self):
        ...
        file_handler = logging.FileHandler(os.path.join(self.util.get_config_path(), 'extremecooling4Linux.log'))
...
if __name__ == '__main__':
    if getattr(sys, '_MEIPASS', ''):
        logger.info('running ExtremeCooling4Linux as PyInstaller bundle')
    else:
        logger.info('running ExtremeCooling4Linux in a normal Python process')
    app = App()
    util = Util()
    ...
    try:
        arg1 = sys.argv[1]
        logger.info('running ExtremeCooling4Linux as a command line program')
        if os.getuid() == 0:
            app.ec.inicializate()
            try:
                if arg1 == "enable":
                    ...
                    logger.info("enable, disable, check, status, and change-state are the only valid arguments for ExtremeCooling4Linux")
            except NoSupportedExtremeCoolingException as e:
                logger.error(e)
        else:
            logger.info("You don't have permissions to execute ExtremeCooling4Linux")
    except IndexError:
        logger.info('running ExtremeCooling4Linux as gui program')
        app.execute()
